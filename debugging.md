...
* Maybe you set the command to conditional accidentally, and it should be unconditional, double-check that
...
* Make sure file extensions are not hidden ([Windows](http://i.imgur.com/FJ9x9Yg.png), [Mac](http://i.imgur.com/xDdYbXL.png)), otherwise a :file: file that looks like `function.mcfunction` might actually be `function.mcfunction.txt`
* [Check what errors you are receiving in the game log](https://imgur.com/a/HWQUUjX) (this isn't chat). It can be enabled in the launcher settings.
* Commands cannot start with a `/` in functions (you'll get told this if you check the game log)
* Macro lines **must** start with `$` and the values are specified using `$(example)`
  * Macro commands **must** contain at least one insert macro
  * Inserting from a :list: list/array or NBT path is not allowed
* Use a plain text editor (Notepad, Notepad++, Sublime, Code), **not** a word processor or rich text editor (Microsoft Word, Wordpad, Textedit)
* Make sure your file's encoding is UTF-8 (without BOM) - this is not the default in many programs! ([Notepad](http://i.imgur.com/R4yFjAQ.png), [Notepad++](http://i.imgur.com/8AsDJ3F.png), [Sublime](http://i.imgur.com/63rsYOB.png), [VSCode](http://i.imgur.com/dmOqy0y.png))
* The namespace :folder: folder is not optional, functions should **not** be directly inside `data/functions/`, they must be in `data/<namespace>/functions/` (or in `data/<namespace>/function/` in 1.21+)
* Don't forget to save changes if editing directly the datapack (with programs like VScode), normally `ctrl+s` (Windows/Linux) or `cmd+s` (Mac)
* Don't forget to use `/reload` to reload the functions after making changes
  ...
  * When editing world generation and other experimental features you need to leave and rejoin the world in order to save changes (or restart the server).
...
* Recursive/looping functions will run `maxCommandChainLength`/`max_command_chain_lenght` commands in one tick, then stop, the default value of this gamerule is `65536`
  * To check if a function did **not** end (so it has been cut off by this gamerule) you can set a scoreboard value of a [fakeplayer](wiki/questions/fakeplayer) to 1, run all commands and set it to 0 after that (that command should be the last one in the whole tick). In the next tick you can check if the value is 1 or 0, if it's 1, the function has been cutted.
* Make sure the tick/load function tag is specifying the correct function
...
* In snapshot [24w21a](https://www.minecraft.net/en-us/article/minecraft-snapshot-24w21a) (for 1.21) some registry types that used legacy datapack directory names (based on plural name of the element) have been renamed to match the registry name. Affected directories:
...
* In some situations, selectors have player bias (will select players first, even if another entity is closer) or sender bias (will select the command's executor first, even if another entity is closer to the specified `x,y,z`). 
...
* Try using `/say` to check whether it's the selector or the rest of your command that's not working, make sure you have chat enabled in client options
...
* [Selector arguments](http://minecraft.wiki/Commands#Target_selector_arguments) usually specify a maximum value, and adding `m` (or `_min` for scores) will specify a minimum value. E.G (in older versions):
     ...
     * `score_x=5` will select anyone with a score x of 5 and **below**, `score_x_min=5` will select anyone with a score x of 5 and **above**
...
* `as` and `at` have a difference in [command context](/wiki/questions/commandcontext). `as` changes the executor entity and `at` changes the position.
...
* Some commands like `/data` or `/damage` in Java edition can only select one target
  * The command `/data merge entity @e[type=armor_stand] {Invisible:1b}` will **not** work
    * You can use `/execute` to select more than one entity, so `/execute as @e[type=armor_stand] run data merge entity @s {Invisible:1b}` will work and modify the data of every loaded :armor_stand: `armor_stand`
  * You can add a `limit`, so `/data merge entity @e[type=armor_stand,limit=1,sort=nearset] {Invisible:1b}` will work and will select the nearest :armor_stand: `armor_stand`
...
* When testing data:
   ...
   * In a :list: list, the same element can be matched multiple times. E.G: `{Motion:[0.0,1.7,2.5]}` matches `/testfor @e {Motion:[0.0,0.0,0.0]}`, as all `0.0`'s find the first `0.0`
...
* Minecraft generally won't "fix" data inside an item's `tag` tag; if you give an item with `{ench:[{id:10,lvl:1}]}`, `id` and `lvl` will stay (and need to be tested) as :int: integers, even though they're normally shorts
...
* If you need to include quotes in a :string: string, you'll need to ["escape"](/wiki/escaping) them by putting \ in front of them. E.G: `{Command:"/say My name is \"\"!"}`
...
* [Generators are handy](https://mcstacker.com) you can find some on the [Java resources page](/wiki/resources) or on the [Bedrock resources page](/wiki/bcresources)
...
* As of 1.12, :string: strings containing characters that aren't `a`-`z`, `A`-`Z`, `0`-`9`, `.`, `_`, `+`, or `-` now need to be quoted. 
    * For example, `Command:say test` contains a space, so it would need to be `Command:"say test"`
    * Make sure to properly escape any quotation marks contained in your command when doing this
* As of 1.12, :list: lists can no longer have indices. 
    * For example, `Lore:[0:"aaa",1:"bbb",2:"ccc"]` would need to be `Lore:["aaa","bbb","ccc"]`
* As of 1.12, array syntax has changed. Most notable on the :int_array: integer array in firework rocket colors where `Colors:[7310,31]` would now need to be `Colors:[I;7310,31]`
...
* Check that the :json: JSON syntax is valid with a [JSON validator](http://jsonlint.com/)
...
* Macs add invisible layers of :folder: folders to zip files. These will stop zipped resources from working
* If you're working with a zip file rather than a :folder: folder, you'll need to update the zip file after saving the files (working in a :folder: folder is easier)
...
* In order to people to use the `/trigger` command, the scoreboard **must** be enabled for that player, you can enable a trigger with this command: `/scoreboard players enable <player_selector> <objective>`.
  ...
  * When you use the `/trigger` command that scoreboard no longer is enabled
* Scoreboard values are linked to the username **not** to the player `UUID`
* When you pick up an item and drop it again, it will lose all the scoreboard values (and tags)
