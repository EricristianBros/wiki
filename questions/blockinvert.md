...
Because we can't access the `SuccessCount` in bedrock we will need to use a comparator pointing into a block that has a wall redstone torch to invert the output. Keep in mind that redstone is not run every tick. Furthermore, for `if` conditions we can use `unless` instead. For example
